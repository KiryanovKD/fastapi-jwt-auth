...
def client():
    ...
    async def websocket_jwt_required(websocket: WebSocket, token: str = Query(...), Authorize: AuthJWT = Depends()):
    ...
    async def websocket_jwt_optional(websocket: WebSocket, token: str = Query(...), Authorize: AuthJWT = Depends()):
        await websocket.accept()
        try:
...
@pytest.mark.parametrize(
    "url", ["/jwt-required-cookies", "/jwt-refresh-required-cookies", "/fresh-jwt-required-cookies"]
)
...
@pytest.mark.parametrize(
    "url",
    ["/jwt-required-cookies", "/jwt-refresh-required-cookies", "/fresh-jwt-required-cookies", "/jwt-optional-cookies"],
)
